...
// std::vector<float> pitches {C4, Csharp4, D4, Dsharp4, E4, F4, Fsharp4, G4, Gsharp4, A4, Asharp4, B4, C5};
...
void updateToss(Ball ball, int index) {
    ball.yRate -= 0.01;
    ball.y += ball.yRate;
    ball.z -= 0.2;
    if (ball.y < -25) {
...
public:
  ...
  void init() override
  {
    ...
    createInternalTriggerParameter("type", 0.0, 0.0, 1.0);
  }
  ...
  void onTriggerOn() override { 
    mAmpEnv.reset(); 
    float num = getInternalParameterValue("type");
    }
...
public:
  ...
  bool onKeyDown(Keyboard const &k) override
  {
    ...
    case '1': {
        playNote(C4, 0, 100, 0.01, 0.0);
        return false;
    }

    case '2': {
        playNote(E4, 0, 1, 0.3, 0.0);
        return false;
    }

    case '3': {
        playNote(E4, 0, 1, 0.3, 1.0);
        return false;
    }

    case 'a': {
      // std::cout << "a pressed!" << std::endl;
      Ball newBall1 {255, 50, 50, -10, 0, -30, 1, 0.3, "toss"};
      Balls.push_back(newBall1);
      Ball newBall2 {50, 255, 50, 0, 0, -30, 1, 0.3, "toss"};
      Balls.push_back(newBall2);
      Ball newBall3 {50, 50, 255, 10, 0, -30, 1, 0.3, "toss"};
      Balls.push_back(newBall3);
      return false;
    }

    case 's' : {
        Ball newBall {rand()%100/100.0, 1, 1, rand()%200/10.0-10.0, 12, rand()%200/10.0-55.0, 1, 0, "drop"};
    ...
    case 'z': {
        ...
        playNote(C4, 0);
        Ball newBall {rand()%100/100.0, 1, 1, -7, -10, -30, 1, 0.5, "toss"};
        Balls.push_back(newBall);
        return false;
    }
    ...
    case 'x': {
        ...
        playNote(D4, 0);
        Ball newBall {rand()%100/100.0, 1, 1, -5, -10, -30, 1, 0.5, "toss"};
        Balls.push_back(newBall);
        return false;
    }
    ...
    case 'c': {
        ...
        playNote(E4, 0);
        Ball newBall {rand()%100/100.0, 1, 1, -3, -10, -30, 1, 0.5, "toss"};
        Balls.push_back(newBall);
        return false;
    }
    ...
    case 'v': {
        ...
        playNote(F4, 0);
        Ball newBall {rand()%100/100.0, 1, 1, -1, -10, -30, 1, 0.5, "toss"};
        Balls.push_back(newBall);
        return false;
    }
  ...
  void playNote(float freq, float time = 0, float duration = 1, float amp = 0.2, float type = 0.0, float attack = 0.1, float decay = 0.1)
  {
    ...
    voice->setInternalParameterValue("type", type);
